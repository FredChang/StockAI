...
window.showStockDetails = async (id) => {
  ...
  triggerAutomaticAnalysis(fullInfo);
...
async function triggerAutomaticAnalysis(fullInfo) {
  const panel = document.getElementById('analysis-output-panel');
  if (!panel) return;

  panel.style.display = 'block';
  panel.innerHTML = '<div style="color: var(--text-secondary); text-align: center; padding: 10px;">🧠 智慧模型分析中，請稍候...</div>';

  const apiKey = localStorage.getItem('gemini_api_key');
  if (apiKey) {
    try {
      const formatVal = (v, suffix = '') => (v == null || isNaN(v)) ? '無' : `${v}${suffix}`;
      const formatRev = (v) => {
        if (v == null || isNaN(v)) return '無';
        return `${(v * 1000).toLocaleString()} 元`;
      };
      
      const payload = {
        contents: [{
          parts: [{
            text: `你是一個專業的台灣股市分析專家。請分析這家公司 (${fullInfo.name}, 代號 ${fullInfo.code}) 的最新財務指標：
* 本益比 (P/E): ${formatVal(fullInfo.pe)}
* 股價淨值比 (P/B): ${formatVal(fullInfo.pb)}
* 現金殖利率: ${formatVal(fullInfo.dy, '%')}
* 單月營收: ${formatRev(fullInfo.rev)} (營收年月: ${fullInfo.revYm || '無'}, 年增率: ${formatVal(fullInfo.revYoY, '%')}, 月增率: ${formatVal(fullInfo.revMoM, '%')})
* 累計營收年增率: ${formatVal(fullInfo.revCumYoY, '%')}

請提供一個 200 字以內的簡短投資診斷，包含以下三個簡短段落，重點處加粗：
1. **估值與股利率評估**：說明估值是否便宜，殖利率是否具備吸引力。
2. **營收與成長動能**：分析營收成長率，說明公司短期與中長期營運動能。
3. **綜合診斷與操作建議**：給出基於上述財務指標的具體投資建議（短線/長線操作）。
...
請使用繁體中文，格式化為三個段落。`
      ...
      if (!res.ok) {
        throw new Error(`Gemini API 請求失敗，狀態碼 ${res.status}。`);
      }

      const data = await res.json();
      const reply = data.candidates?.[0]?.content?.parts?.[0]?.text;
      if (reply) {
        panel.innerHTML = formatMarkdown(reply);
      } else {
        throw new Error("無法解析 Gemini API 的回傳結果。");
      }
    } catch (e) {
      console.error(e);
      panel.innerHTML = `<div style="color: #ef4444; padding: 5px; margin-bottom: 10px;">❌ AI 分析錯誤: ${e.message}<br><span style="color: var(--text-secondary); font-size: 0.75rem;">(已自動切換回本機智慧分析)</span></div>`;
      await new Promise(r => setTimeout(r, 1500));
      runLocalRuleAnalysis(fullInfo, panel);
    }
  } else {
    runLocalRuleAnalysis(fullInfo, panel);
  }
...
function runLocalRuleAnalysis(s, panel) {
  const hasPE = s.pe != null && !isNaN(s.pe);
  const hasPB = s.pb != null && !isNaN(s.pb);
  const hasDY = s.dy != null && !isNaN(s.dy);
  const hasYoY = s.revYoY != null && !isNaN(s.revYoY);

  let valuationText = "";
  if (hasPE) {
    if (s.pe <= 0) {
      valuationText = `目前處於虧損或無盈餘狀態。`;
    } else if (s.pe < 12) {
      valuationText = `目前本益比為 **${s.pe.toFixed(1)}倍**，估值處於**歷史偏低區間**，安全邊際高。`;
    } else if (s.pe <= 20) {
      valuationText = `目前本益比為 **${s.pe.toFixed(1)}倍**，估值合理，反映穩定的市場預期。`;
    } else {
      valuationText = `目前本益比為 **${s.pe.toFixed(1)}倍**，**估值偏高**，市場已賦予較高的成長溢價。`;
    }
  } else {
    valuationText = `暫無本益比數據。`;
  }
  if (hasPB) {
    valuationText += ` 股價淨值比為 **${s.pb.toFixed(2)}倍**${s.pb < 1.2 ? '（低於1.2倍，資產折價具吸引力）' : ''}。`;
  }

  let dividendText = "";
  if (hasDY) {
    if (s.dy >= 5.0) {
      dividendText = `現金股利殖利率高達 **${s.dy.toFixed(2)}%**，屬於**高股息定存股**，具備優異的下檔防禦性與現金流回報。`;
    } else if (s.dy >= 3.0) {
      dividendText = `現金股利殖利率為 **${s.dy.toFixed(2)}%**，殖利率表現**中等穩健**，股利配發符合一般市場水準。`;
    } else if (s.dy > 0) {
      dividendText = `現金股利殖利率為 **${s.dy.toFixed(2)}%**，殖利率偏低，公司可能傾向於保留盈餘進行擴張再投資。`;
    } else {
      dividendText = `目前殖利率為 **0%**，公司未配發股利或側重於資本利得成長。`;
    }
  } else {
    dividendText = `暫無殖利率數據。`;
  }

  let revenueText = "";
  if (hasYoY) {
    if (s.revYoY >= 20.0) {
      revenueText = `最新單月營收年增率達 **${s.revYoY.toFixed(1)}%**，成長動能**極為強勁**！反映產業景氣旺盛或產品競爭力強。`;
    } else if (s.revYoY >= 0) {
      revenueText = `最新單月營收年增率為 **${s.revYoY.toFixed(1)}%**，營運呈現**溫和成長**。`;
    } else if (s.revYoY < -15.0) {
      revenueText = `最新單月營收年增率為 **${s.revYoY.toFixed(1)}%**，**營收大幅衰退**，需警戒是否有行業周期逆風或公司營運瓶頸。`;
    } else {
      revenueText = `最新單月營收年增率為 **${s.revYoY.toFixed(1)}%**，營收輕微衰退。`;
    }
  } else {
    revenueText = `暫無最新營收成長率數據。`;
  }

  let recText = "";
  const score = (s.pe > 0 && s.pe < 15 ? 2 : 0) + (s.dy >= 4.5 ? 2 : 0) + (s.revYoY >= 15 ? 3 : 0);
  if (score >= 5) {
    recText = `本標的在**估值安全**與**成長動能**上均表現優異。建議**長線分批佈局**或逢低吸納，並留意技術面多頭排列順勢操作。`;
  } else if (s.revYoY > 15) {
    recText = `本標的短期營收動能充沛，但估值偏高或殖利率較低，屬**強勢動能股**。建議以技術線型（如偏離MA60、趨勢強度）進行**短線價差操作**，嚴設停損。`;
  } else {
    recText = `本標的基本面穩健但爆發力稍顯不足。操作上建議作為**穩健型存股**長期持有，或是等候月營收年增率重新翻正拉升後，再行介入。`;
  }

  panel.innerHTML = `
    <strong>【估值評估】</strong><br>
    ${valuationText}<br><br>
    <strong>【營收動能】</strong><br>
    ${revenueText}<br><br>
    <strong>【股利與存股安全度】</strong><br>
    ${dividendText}<br><br>
    <strong>【診斷與操作建議】</strong><br>
    ${recText}
  `;
}
